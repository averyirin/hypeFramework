...
function hj_framework_prepare_form($form_name, $params = array()) {
	...
	//check if container entity belongs to a group
	if($params['container_guid']){
		$containerGuid = $params['container_guid'];
	}
	else{
		$containerGuid = $params['entity']->guid;
	}
	$parents = elgg_get_entities_from_relationship(array(
		'relationship' => 'descendant',
		'relationship_guid' => $containerGuid,
	));

	for($i=1;$i <= count($parents);$i++){
		if(elgg_instanceof($parents[$i-1], "group")){
			$parent_group = $parents[$i-1];
		}
	}
			...
			if (is_string($options)) {  // field options are set as a callback function
				...
				if($name == 'access_id'){
					if(elgg_instanceof($parent_group, "group")){
						if($params['access_id'] = -1){
							$options['value'] = $parent_group->group_acl;
							$options['admin_option'] = array($parent_group->group_admin_acl => "Group Admins: ".$parent_group->name);
						}
					}
				}
			}
			//$options['name']['access_id'] = $params['access_id'];
			//var_dump($options);
